  ...
  const UpdateCam = () => {
    ...
    const { mutateAsync } = useMutation({
      mutationFn: async (info) => {
        const { data } = await axiosSecure.patch(`/update-cam/${id}`, info);
        return data;
      },
      onSuccess: () => {
        setBtnSpin(false);
      ...
      onError: (error) => {
        ...
        setBtnSpin(false);
        toast.error(`Error updating campaign. Please try again. ${error.message}`);
      },
    });
  
    const onSubmit = async (formData) => {
      setBtnSpin(true);
      const uploadImag = new FormData();
      uploadImag.append("image", img);
      ...
      <div className="max-w-4xl mx-auto mt-10 p-6 bg-white shadow-md rounded-lg">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 grid-cols-1 gap-4">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div>
                  ...
                  />
                  {errors.maximum_amount && <span className="text-red-500">{errors.maximum_amount.message}</span>}
                </div>
          ...
          <Button
            disabled={btnSpin}
            type="submit"
            className="mt-10 bg-primary hover:bg-primary-dark transition-colors duration-300"
            fullWidth
          >
            {btnSpin ? <SyncLoader size={8} color="#FFFFFF" /> : 'Update Campaign'}
          </Button>
        </form>
      </div>
    );
